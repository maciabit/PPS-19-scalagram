...
class KeyboardSuite extends AnyFunSuite with BeforeAndAfterEach {
  ...
  test("A message with a single button Response Keyboard can be sent") {
    ...
    val message = bot.sendMessage(groupChatId, "Single button keyboard", replyMarkup = keyboard)
  ...
  test("A message with a column of buttons Response Keyboard can be sent") {
    ...
    val message = bot.sendMessage(groupChatId, "Row of buttons keyboard", replyMarkup = keyboard)
    assert(message.isSuccess)
  }
  ...
  test("A message with a row of buttons Response Keyboard can be sent") {
    ...
    val message = bot.sendMessage(groupChatId, "Column of buttons keyboard", replyMarkup = keyboard)
    assert(message.isSuccess)
  }
  ...
  test("A message with a grid of buttons Response Keyboard can be sent") {
    ...
    val message = bot.sendMessage(groupChatId, "Grid of buttons keyboard", replyMarkup = keyboard)
    assert(message.isSuccess)
  }

  test("A Response Keyboard can be removed") {
    val keyboard = Some(ReplyKeyboardRemove())
    val message = bot.sendMessage(groupChatId, "Removing response keyboard", replyMarkup = keyboard)
    assert(message.isSuccess)
  }
  ...
  test("URL Buttons, Callback Buttons and Switch to Inline Buttons can be sent in an Inline Keyboard") {
    ...
    val message = bot.sendMessage(groupChatId, "Various Inline Buttons", replyMarkup = keyboard)
    assert(message.isSuccess)
  }
  ...
  test("A message that displays a reply interface to the user can be sent") {
    ...
    val message = bot.sendMessage(groupChatId, "You all must answer me", replyMarkup = keyboard)
    assert(message.isSuccess)
  }
}
