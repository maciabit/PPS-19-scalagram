...
class KeyboardSuite extends AnyFunSuite with BeforeAndAfter {
  ...
  test("A message with a Response Keyboard with a single button can be sent"){
    ...
    assert(TestUtils.retry(SendMessage().sendMessage(chatId = Left("-1001286594106"), text = "Single button test", replyMarkup = Some(a)))(3).isSuccess)
  }
  ...
  test("A message with a Response Keyboard with a row of button can be sent"){
    ...
    assert(TestUtils.retry(SendMessage().sendMessage(chatId = Left("-1001286594106"), text = "Row of button test", replyMarkup = Some(a)))(3).isSuccess)
  }
  ...
  test("A message with a Response Keyboard with a column of button can be sent"){
    ...
    assert(TestUtils.retry(SendMessage().sendMessage(chatId = Left("-1001286594106"), text = "Column of button test", replyMarkup = Some(a)))(3).isSuccess)
  }
  ...
  test("A message with a response keyboard allowing user to send his location and phone number can be sent (ONLY FOR PRIVATE CHAT)"){
    ...
    assert(TestUtils.retry(SendMessage().sendMessage(chatId = Left("263890809"), text = "Column of button test", replyMarkup = Some(a)))(3).isSuccess)
  }
  ...
  test("The Response keyboard can be removed"){
    ...
    assert(TestUtils.retry(SendMessage().sendMessage(chatId = Left("-1001286594106"), text = "Removing response keyboard", replyMarkup = Some(a)))(3).isSuccess)
  }

  /*ForceReplyTest*/
  test("A message that display a reply interface to the user can be sent"){
    val a = (ForceReply() : ReplyMarkup)
    assert(TestUtils.retry(SendMessage().sendMessage(chatId = Left("-1001286594106"), text = "You all must answer me", replyMarkup = Some(a)))(3).isSuccess)
  }
}
